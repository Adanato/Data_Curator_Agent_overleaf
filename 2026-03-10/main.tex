...
We adopt the minimal effective set from~\cite{smellyToolDesc2026} and rewrote all 20+ tool descriptions: \textbf{Purpose} (what the tool does + what it returns), \textbf{Guidelines} (when to use it + ordering constraints), \textbf{Limitations} (failure modes + performance constraints).
...
\hline
\textbf{Before (ad-hoc, implementation-leaking):} \\[2pt]
\texttt{"""Filter a dataset by image integrity checks.} \\
\texttt{Use case: Run before text quality filtering...} \\
\texttt{Implementation: Iterates all rows checking image} \\
\texttt{decodability, minimum side length (200px)...} \\
\texttt{Output: JSON with dataset, original\_rows, kept..."""} \\[4pt]
\hline
\textbf{After (Purpose / Guidelines / Limitations):} \\[2pt]
\texttt{Purpose: Remove rows whose images are corrupt,} \\
\texttt{too small (<200px), or extreme aspect ratios (>3:1).} \\
\texttt{Returns kept/rejected counts and per-reason breakdown.} \\[2pt]
\texttt{Guidelines: Run on each dataset after profile\_datasets} \\
\texttt{and before text-based filters. Only requires dataset\_id} \\
\texttt{--- thresholds are fixed. Safe on all dataset types.} \\[2pt]
\texttt{Limitations: Requires loading full image data into} \\
\texttt{memory. Slow on large datasets with many shards.} \\
\texttt{Does not assess image content quality --- only} \\
\texttt{structural properties.} \\[2pt]
\hline
\end{tabular}
\caption{Before/after tool description for \texttt{vlm\_filter}. The old format exposes internals (``iterates all rows'') and lacks ordering guidance or failure modes. The new format follows the minimal effective component set from~\cite{smellyToolDesc2026}: Purpose is essential; Examples are omitted (no significant effect in ablation).}
\label{fig:tool-desc}
\end{figure}
