...
\centering
\small
\caption{LLaVA-1.5-7B zero-shot baselines. All values are from published papers unless noted. We did not run our own zero-shot evaluation.}
\label{tab:judge-sanity}
\begin{tabular}{lcll}
\toprule
\textbf{Benchmark} & \textbf{Baseline} & \textbf{Source} & \textbf{Note} \\
\midrule
MMStar & 30.3 & \cite{chen2024mmstar} & from MMStar paper, Table~5 \\
OCRBench & 297 & \cite{liu2023ocrbench} & from paper (VLMEvalKit reports 317) \\
HallusionBench & 27.6 & VLMEvalKit & composite $(\text{aAcc}+\text{fAcc}+\text{qAcc})/3$; no paper value for 7B \\
MMMU\_DEV\_VAL & 36.4 & \cite{yue2024mmmu} & from MMMU paper (lmms-eval reports 35.3) \\
MMVet & 31.1 & \cite{liu2024llava15} & from LLaVA-1.5 paper, Table~4 \\
MathVista\_MINI & N/A & --- & original LLaVA-13B scores 26.1~\cite{lu2024mathvista}; no paper value for 1.5-7B \\
\bottomrule
\end{tabular}
\end{table}
